...
Tóm lại là RNN rất tốt trong việc xử lý chuỗi dữ liệu để thực hiện dự đoán nhưng lại gặp phải vấn đề là short-term memory, LSTM và GRU được tạo ra để giải quyết short-term memory bằng cách sử dụng cơ chế được gọi là cổng. Các cổng chỉ là mạng neural dùng để điều chỉnh luồng thông tin thông qua chuỗi dữ liệu. LSTM và GRU còn được dùng trong các ứng dụng khác như speech recognition, speech synthesis, natural language understanding, image captioning, ...
